...
class newAttacker(Strategy):
	...
	#ATTENTION parametrizar tudo do field
        ...
        self.defend.add_field(
            algorithms.fields.PointField(
                self.match,
                target = (0.75, 0.65), #parametrizar
                radius = 0.2, # 20cm
                decay = lambda x: x**2,
                field_limits = [0.75*2 , 0.65*2], #parametrizar
                multiplier = 1
            )
        )
	...
	Mat
        ...
        all_robots = self.match.opposites + self.match.robots
        possession = get_ball_possession(all_robots, self.match)
