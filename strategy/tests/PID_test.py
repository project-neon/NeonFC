...
class PID_Test(Strategy):
    def __init__(self, match, plot_field=False):
        ...
        self.circuit = [(0.375, 0.25), (1.125, 0.25), (1.125, 1.05), (0.375, 1.05)]#, (.4, .90), (.4, .40)]
    ...
    def decide(self):
        ...
        desired = behaviour.compute([self.robot.x, self.robot.y])
        desired = math.atan2(desired[1], desired[0])

        desired_dl = behaviour.compute([self.robot.x + self.dl*math.cos(self.robot.theta),
                                         self.robot.y + self.dl*math.sin(self.robot.theta)])
        desired_dl = math.atan2(desired_dl[1], desired_dl[0])
