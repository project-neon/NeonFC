...
class LeftDefenser(Strategy):
    ...
    def start(self, robot=None):
        ...
        self.up.add_field(
            algorithms.fields.LineField(
                self.match,
                target = (self.x, self.sa_h + self.sa_y),
                theta = 0,
                line_size = self.field_h - self.sa_w,
                line_dist = 0.1,
                line_dist_max = self.field_h,
        ...
        self.redeploy.add_field(
            algorithms.fields.TangentialField(
                self.match,
                target = (self.x, self.field_h/2 + 0.1),
                radius = 0.00001,
