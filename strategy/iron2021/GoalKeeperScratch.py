...
class GoalKeeper(Strategy):
    ...
    def start(self, robot=None):
        ...
        self.circle = algorithims.fields.PotentialField(
            self.match,
            name="{}|CircleBehavior".format(self.__class__)
        )
        ...
        self.push.add_field(
            algorithims.fields.PointField(
                self.match,
                target = lambda m : (m.ball.x - 0.05, m.ball.y),
                radius = 0.05, # 30cm
                decay = lambda x: 1,
                field_limits = [0.75* 2 , 0.65*2],
        ...
        self.circle.add_field(
		    algorithims.fields.TangentialField(
		        ...
		        radius_max = 0.7,
		        clockwise = True if self.match.ball.y > self.robot.y else False,
		        decay = lambda x: 1,
		        field_limits = [0.75* 2 , 0.65*2],
		        multiplier = 1.2
		    )
    ...
    def decide(self):
        ...
        goal_area = [-0.00, 0.20, 0.20, 0.70]
        
        min_angle_ball_to_goal = -math.atan2((self.match.ball.y - 0.55), (self.match.ball.x - 0.0))
        ...
        elif point_in_rect(ball ,goal_area) and (min_angle_to_goal <= 0.60) and (self.match.ball.x < self.robot.x):
            behaviour = self.circle
