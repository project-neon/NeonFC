...
class LeftWing(Strategy):
    ...
    def start(self, robot=None):
        ...
        self.defend.add_field(
            algorithms.fields.PointField(
                self.match,
                target = lambda m: (m.ball.x, (self.sa_y + self.sa_h + 0.2)),
                radius = .075,
                decay = lambda x: x**6,
                multiplier = 1
            )
        ...
        self.attack.add_field(
            algorithms.fields.PointField(
                self.match,
                target = lambda m: (m.ball.x - 0.4, (self.sa_y + self.sa_h + 0.2)),
                radius = .075,
                decay = lambda x: x**6,
                multiplier = 1
            )
    ...
    def decide(self):
        ...
        if ball.y > self.sa_y + self.sa_h + 0.1 and ball.x > self.robot.x:
